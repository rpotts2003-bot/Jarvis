...
def test_recognize_native_success(monkeypatch, tmp_path):
    ...
    text, err, conf = windows_sr.recognize_native(timeout_s=3.0, runner=runner)
...
def test_recognize_native_timeout(monkeypatch, tmp_path):
    ...
    text, err, conf = windows_sr.recognize_native(runner=runner)
    assert text is None
    assert err == "timeout"
    assert conf is None
...
def test_make_mic_hear_ptt_fallback_on_timeout(monkeypatch):
    """Listen: one Google retry after Windows SR timeout/empty (speech likely)."""
    calls = {"google": 0}
    ...
    hear = make_mic_hear(mode="ptt")
    assert hear is not None
    assert hear() == "from google retry"
    assert calls["google"] == 1
    assert getattr(hear, "last_backend", None) == "google"


def test_make_mic_hear_wake_no_fallback_on_timeout(monkeypatch):
    """Wake chunks stay Windows-first — no Google retry on timeout."""
    calls = {"google": 0}

    def fake_native_hear_factory(*, on_level=None, timeout_s=8.0, mode="wake", runner=None):
        def hear():
            hear.last_error = "timeout"
            hear.last_peak = 0.0
            return None
    ...
    def fake_google(*, on_level=None, mode="wake", duration_s=None, wait_speech_s=None):
        def hear():
            calls["google"] += 1
...
def test_wake_mode_uses_native_when_available(monkeypatch):
    ...
    def fake_native(*, on_level=None, timeout_s=8.0, mode="wake", runner=None):
        called["native"] += 1
